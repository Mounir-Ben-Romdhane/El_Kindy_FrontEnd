...
const AssignmentsComponent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const assignmentsResponse = await axios.get(`https://el-kindy-project-backend.onrender.com/auth/assignments/${decodeToken.course}/${decodeToken.id}`, {
          headers: {
        ...
        const teachersResponse = await axios.get('https://el-kindy-project-backend.onrender.com/auth/teachers');
        setTeachers(teachersResponse.data);

        const coursesResponse = await axiosPrivate.get(`https://el-kindy-project-backend.onrender.com/auth/course/${decodeToken.id}`);
  ...
  const handleDownloadFile = async (filePath) => {
    try {
      const response = await axios.get(`https://el-kindy-project-backend.onrender.com/${filePath}`, {
        responseType: 'blob',
      });
...
console.log("asa",decodeToken.id)
    ...
    const response = await axios.post(`https://el-kindy-project-backend.onrender.com/api/upload/${assignmentId}`, formData, {
      headers: {
    ...
    console.log("Submit file response:", response.status);
      ...
      <section className="pt-4">
        <div className="container">
          <div className="row">
            ...
            <div className="card  bg-transparent border-2 rounded-3 p-4 ">
              ...
              {filteredAssignments.map((assignment, index) => (
                <div key={assignment.id} className="assignment-section row bg-transparent border rounded-3 mx-4  my-2 p-3">
                  <div className="col-md-3">
                    <div style={{ background: 'linear-gradient(rgba(236, 231, 225, 0.25), rgba(18, 91, 193, 0.93))' }}>
                      <img
                        src={ `https://el-kindy-project-backend.onrender.com/assets/CurvedSopranoAltoTenorSaxophone.jpg`}
                  ...
                  <div className="col-md-9">
                    <div>
                      <h3 className="card-title"><a href={`https://el-kindy-project-backend.onrender.com/assets/${assignment.picturePath}`} target="_blank">{assignment.title}</a></h3>
                      ...
                      <div>
                          ...
                          {assignment.file && (
    <a href={`https://el-kindy-project-backend.onrender.com/assets/${assignment.file}`} download className="btn btn-primary-soft btn-sm mb-0 ms-2">Télécharger à nouveau</a>
)}
